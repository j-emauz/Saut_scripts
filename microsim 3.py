...
# Helper function para funcao check_intersect
...
def laser_model(x_true, tl):
    ...
    # elif check_intersect(P51, P52, pl1, pl2):
    #    laser_scan = get_intersect(P51, P52, pl1, pl2) 
    #   r = np.linalg.norm(pl1-laser_scan)
    else:
...
def update(x_est, E_est, z, R_seg, mapa, g):
    ...
    #mu_ear formato de v, H e R para usar nas equacoes
...
if __name__ == '__main__':
    ...
    while time <= 150:
        ...
        """
        for monkey in range(0, seg_i_f.shape[0]):
            seg_i_f = np.array(seg_i_f)
            point1 = [seg_i_f[monkey, 0], seg_i_f[monkey, 1]]
            point2 = [seg_i_f[monkey, 2], seg_i_f[monkey, 3]]
