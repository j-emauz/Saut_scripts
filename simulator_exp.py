...
"""
Linhas que representam um corredor com 1 m de largura:
P11 = np.array([-3, -3])
P12 = np.array([3, -3])
X1 = [P11[0], P12[0]]
Y1 = [P11[1], P12[1]]
...
class Thresholds:
    #definição dos thresholds que vão ser utilizados para a validação dos segmentos de reta
    def __init__(self):
...
def fitline(pontos):
    """
    Rebece: todos os pontos fornecidos pelo lazer
    Retorna: alpha e r do segmento reta
    """
...
def compdistpointstoline(xy, alpha, r):
    """
    Rebece: todos os pontos fornecidos pelo lazer(xy), o alpha e r do segmento de reta
    Retorna: a distância dos pontos ao segmento de reta(d)
    """
...
def findsplitposid(d, thresholds):
    """
    Rebece: as distâncias dos pontos ao segmento de reta(d) e as condições que definem os segm de reta(thresholds)
    Retorna: -1 se não é necessário split, 1 se p split acontece logo no ponto inicial, N-2 se existir split noutro ponto
    """
...
def findsplitpos(xy, alpha, r, thresholds):
    """
    Rebece: todos os pontos fornecidos pelo lazer(xy), o alpha, r do segmento de reta e os thresholds
    Chama as funções: compdistpointstoline e findsplitposid
    Retorna: o index onde irá ocorrer split
    """
...
def splitlines(xy, startidx, endidx, thresholds):
    """
    Rebece: todos os pontos fornecidos pelo lazer(xy), startidx, endix(dos pontos) e os thresholds
    Chama as funções: fitline, findsplitpos e a própria função splitlines(caso split seja necessário)
    Retorna: alpha, r e idx de todos os segmentos reta
    """
    N = endidx - startidx + 1
...
def mergecolinear(xy, alpha, r, pointidx, thresholds):
    """
    Rebece: todos os pontos fornecidos pelo lazer(xy), alpha e r de todos os segm, pointidx e os thresholds
    Chama as funções: fitline, findsplitpos
    Retorna: alphaOut, rOut, pointidxOut (de todos os segm, pós merge)
    """
    z = [alpha[0, 0], r[0, 0]]
...
def pol2cart(theta, rho):
    """
    Transforma as coordenadas polares dos pontos lidos pelo lazer em coordenadas cartesianas

    Rebece: theta e rho de todos os pontos lidos pelo lazer
    Retorna: x e y de todos os pontos lidos pelo lazer
    """
...
def extractlines(theta, rho, thersholds):
    """
    Ajusta os pontos fornecidos pelo lazer a sgmentos de reta

    Rebece: theta e rho de todos os pontos lidos pelo lazer, e os thresholds dos segmentos
    Chama as funções: pol2chart, splitlines, mergecolinear
    Retorna: z, R_seg e os segmeds(idx dos pontos finais de cada segmento)
    """
